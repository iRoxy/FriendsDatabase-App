...
public class AddActivity extends FragmentActivity {
        ...
        mButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(isValid()){
                    ...
                    Log.d(LOG_TAG, "record id returned: " +  returned.toString());
                    Intent intent = new Intent(AddActivity.this, MainActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    startActivity(intent);
                    finish();
                } else{
                    Toast.makeText(getApplicationContext(), "Please ensure you entered valid data", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    ...
    private boolean isValid()
    {
        if(mNameTextView.getText().length() == 0 ||
                mPhoneTextView.getText().length() == 0 ||
                mEmailTextView.getText().length() == 0)
        {
            return false;
        }
